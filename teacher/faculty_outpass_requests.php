...
if (strpos($rid, 'F') !== 0) {
    echo "<script>alert('Access denied! Only Faculty can access this page.'); window.location.href='rlogin.php';</script>";
    exit();
}
...
// Notification insert (with check)
$role = 'teacher'; // or 'student', based on sender
$sid = $rid; // sender ID
$msg = "New outpass request from SID: $rid";

$notif = $connection->prepare("INSERT INTO notifications (sender_role, sid, message) VALUES (?, ?, ?)");
if ($notif) {
    $notif->bind_param("sss", $role, $sid, $msg);
    $notif->execute();
} else {
    error_log("Notification Prepare failed: " . $connection->error);
    echo "<script>console.warn('Notification insert failed.');</script>";
}
?>
